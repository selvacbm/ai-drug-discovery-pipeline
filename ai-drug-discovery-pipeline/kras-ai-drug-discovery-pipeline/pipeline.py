...
def build_model_pipeline(n_estimators: int):
    descriptor_start = FP_BITS
    descriptor_end = FP_BITS + len(DESCRIPTOR_NAMES)
    # Keep binary Morgan fingerprint bits unchanged and normalize only the
    # continuous RDKit descriptors before regression.
    preprocessor = ColumnTransformer(
        transformers=[
            ("fingerprints", "passthrough", slice(0, FP_BITS)),
            ("descriptors", StandardScaler(), slice(descriptor_start, descriptor_end)),
        ]
    )
    return Pipeline(
        steps=[
...
def train_model(df: pd.DataFrame):
    ...
    random_model = build_model_pipeline(n_estimators=500)
    ...
    scaffold_model = build_model_pipeline(n_estimators=500)
    ...
    final_model = build_model_pipeline(n_estimators=600)
    final_model.fit(X, y)
